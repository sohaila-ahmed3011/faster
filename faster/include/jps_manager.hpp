...
public:
  ...
  void performAStar(Eigen::Vector3d start,Eigen::Vector3d goal);
  void performJPS(Eigen::Vector3d start,Eigen::Vector3d goal);

private:
  double factor_jps_, res_, inflation_jps_, z_ground_, z_max_, drone_radius_;
  int cells_x_, cells_y_, cells_z_, star_counter_ {0}, jps_counter_ {0};
  bool visual_, initiate_path {true}, a_star_completed {false}, Jps_completed {false};
  std::string printInfo_ = "Hybrid A star";
  vec_Vecf<3> path_;  //path generated by the planner
