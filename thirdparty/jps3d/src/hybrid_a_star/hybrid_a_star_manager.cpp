...
//     obsCount = 0;
//     freeCount = 0;
//     unsigned int map_w = std::floor(_cost_map_->info.width);
//     unsigned int map_h = std::floor(_cost_map_->info.height);
//     for (unsigned int w = 0; w < map_w; ++w) {
//         for (unsigned int h = 0; h < map_h; ++h) {
//             auto x = static_cast<unsigned int> ((w + 0.5) * _map_resolution/ _cost_map_->info.resolution);
//             auto y = static_cast<unsigned int> ((h + 0.5) * _map_resolution/ _cost_map_->info.resolution);
...
//             // for (auto node: _cost_map_->data)
//             // {
//             //     std::cout << "The current node has a value of " << node << " and type of " << typeid(node).name() << std::endl;
//             // }
...
//                 // std::cout << "map_w: " << map_w << " map_h: " << map_h << std::endl;
//                 std::cout << " Obstacle value: " << _cost_map_->data[y * _cost_map_->info.width + x] << std::endl;
//                 obsCount++;
//             }
//             else{
//                 freeCount++;
